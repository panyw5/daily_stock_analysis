...
"""
===================================
AkshareFetcher - 补充数据源 (Priority 2)
===================================

数据来源：
1. 东方财富爬虫（通过 akshare 库） - ⚠️ 不可用（服务器问题）
2. 新浪财经接口 - ✅ 可用（财务数据）
3. 腾讯财经接口 - 备选数据源

测试结果（2026-01-29）：
- 可用率：40% ⚠️
- 历史行情（东方财富）：❌ 不可用（Connection aborted）
- 实时行情（东方财富）：❌ 不可用（Connection aborted）
- ETF行情（东方财富）：❌ 不可用（Connection aborted）
- 财务数据（新浪）：✅ 可用（147个字段）
- 资金流向：✅ 可用（13个字段）
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                circuit_breaker.record_failure(
                    source_key, f"HTTP {response.status_code}"
                )
                return None
