...
class HistoryAnalyzer:
    ...
    def analyze_trend(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if all(col in latest for col in ["MA5", "MA10", "MA20", "MA60"]):
            ...
            result["bias_ma10"] = (
                (latest["close"] - latest["MA10"]) / latest["MA10"] * 100
            )
            result["bias_ma20"] = (
        ...
        if "MACD" in latest:
            ...
            result["macd_signal"] = (
                "MACD金叉" if latest["DIF"] > latest["DEA"] else "MACD死叉"
            )
        ...
        if "volume" in df.columns:
            ...
            result["volume_ratio"] = (
                latest["volume"] / avg_volume if avg_volume > 0 else 0
            )

        return result

    def get_ai_analysis(
        self, stock_code: str, analysis: Dict[str, Any], df: pd.DataFrame
    ) -> Optional[str]:
        """
        使用 AI 分析器生成分析报告
...
示例:
  ...
  python history_analysis.py --stock 600519 --no-ai            # 禁用 AI 分析
    ...
    parser.add_argument(
        "--stock", type=str, required=True, help="股票代码，多个代码用逗号分隔"
    )
    ...
    parser.add_argument(
        "--start-date", type=str, help="开始日期，格式：YYYYMMDD 或 YYYY-MM-DD"
    )
    ...
    parser.add_argument("--no-ai", action="store_true", help="禁用 AI 分析（默认启用）")
